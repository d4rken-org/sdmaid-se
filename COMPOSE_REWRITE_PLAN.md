...
- **Automation accessibility overlay — CLOSED** (commit `85fa45052`). The floating overlay shown by `AutomationService` while AppCleaner / AppControl drive the UI was the last XML+ViewBinding holdout. `AutomationControlView` (custom `ConstraintLayout`) and the legacy `MascotView` (custom `FrameLayout` wrapper around `LottieAnimationView` + hat overlay + 0.7×0.6 child scaling + click-counter wiggle/rotate easter egg) were both replaced. The service now hosts a `ComposeView` driven by a single `MutableStateFlow<AutomationOverlayState>`: `updateProgress(...)` writes `overlayState.progress` (replacing the always-on `progress.onEach { setProgress(pd) }` collector that existed alongside it), `changeOptions(...)` writes title/subtitle, cancel is a callback. New `OverlayLifecycleOwner` helper (`LifecycleOwner` + `ViewModelStoreOwner` + `SavedStateRegistryOwner`) drives explicit `ON_CREATE → ON_START → ON_RESUME` at addView and `ON_PAUSE → ON_STOP → ON_DESTROY` at removeView; composition uses `DisposeOnDetachedFromWindow` so it doesn't depend on lifecycle teardown order. `addView` is wrapped so the lifecycle owner is destroyed if either `setContent` or `addView` throws (BadTokenException or anything else); the removeView path is `try/finally` so `controlView` and `overlayLifecycle` are always nulled. Theme is snapshotted once at addView (`ThemeState(themeMode.value(), themeStyle.value())`) — matches legacy `ContextThemeWrapper(R.style.AppTheme)` snapshot semantics. The Compose surface reuses the existing `SdmMascot` (no `isPlaying` plumbing — the early-return on `state.progress == null` removes the mascot from composition entirely). Zero-max guard mirrors `ProgressOverlay.kt`: `Counter`/`Percent` fall back to indeterminate when `current==0L || max==0L`. `Size` keeps the indeterminate spinner with `displayValue` overlay (parity with legacy `progress_text`). Layout robustness: card capped `widthIn(max=600.dp)`, mascot capped `heightIn(max=240.dp)`, secondary text uses `maxLines = 3` only (no fixed height cap). Both `viewBinding = true` flags (`app-common-ui` for `MascotView`, `app-common-automation` for `AutomationControlView`) are gone; **no modules ship `viewBinding = true`**. `vector_mascot_nobroom.xml` kept (still referenced by `app/.../launch_screen.xml`). Easter-egg click counter dropped — debug-only.
...
- ~~Remove ViewBinding build feature~~ ✓ — stripped from all 16 modules. `app/` viewBinding dropped in commit `2fcfb5e09`; the final two holdouts (`app-common-ui` for `MascotView`, `app-common-automation` for `AutomationControlView`) were dropped in commit `85fa45052` after both custom views were converted to Compose. **No modules ship `viewBinding = true`.**
