...
data class RestoreTask(
    ...
    data class Result(
        ...
        override val affectedPkgs: Map<Pkg.Id, AffectedPkg.Action>
            get() = success.associate { it.pkgId to AffectedPkg.Action.RESTORED }

        override val primaryInfo: CaString
            get() = caString {
                val clauses = listOfNotNull(
                    success.size.takeIf { it > 0 }?.let {
                        getQuantityString2(eu.darken.sdmse.appcontrol.R.plurals.appcontrol_restore_result_x, it)
                    },
                ...
                when {
                    clauses.isEmpty() -> getQuantityString2(
                        eu.darken.sdmse.appcontrol.R.plurals.appcontrol_restore_result_x,
                        success.size,
                    )
