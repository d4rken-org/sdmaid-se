...
import eu.darken.sdmse.deduplicator.databinding.DeduplicatorClusterElementMediagroupHeaderBinding
import eu.darken.sdmse.deduplicator.ui.details.cluster.ClusterAdapter
import kotlin.math.roundToLong


class MediaGroupHeaderVH(parent: ViewGroup) :
    ClusterAdapter.BaseVH<MediaGroupHeaderVH.Item, DeduplicatorClusterElementMediagroupHeaderBinding>(
        R.layout.deduplicator_cluster_element_mediagroup_header,
        parent
    ), ClusterAdapter.ClusterItem.VH, ModularAdapter.Module.RecyclerViewLifecycle {

    override val viewBinding = lazy { DeduplicatorClusterElementMediagroupHeaderBinding.bind(itemView) }
    private var handler: Handler? = null
    private var callback: Runnable? = null

    override val onBindData: DeduplicatorClusterElementMediagroupHeaderBinding.(
        item: Item,
        payloads: List<Any>
    ) -> Unit = binding { item ->
    ...
    data class Item(
        ...
        val onViewActionClick: (MediaDuplicate) -> Unit,
