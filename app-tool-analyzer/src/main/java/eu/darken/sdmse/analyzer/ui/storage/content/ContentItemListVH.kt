...
import eu.darken.sdmse.analyzer.databinding.AnalyzerContentItemListVhBinding


class ContentItemListVH(parent: ViewGroup) :
    ContentAdapter.BaseVH<ContentItemListVH.Item, AnalyzerContentItemListVhBinding>(
        R.layout.analyzer_content_item_list_vh,
        parent
    ) {
    ...
    override val viewBinding = lazy { AnalyzerContentItemListVhBinding.bind(itemView) }

    override val onBindData: AnalyzerContentItemListVhBinding.(
        item: Item,
        payloads: List<Any>
    ) -> Unit = binding { item ->
        lastItem = item
