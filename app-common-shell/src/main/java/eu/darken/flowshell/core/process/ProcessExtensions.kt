...
private const val TAG = "RXS:RootKiller"
...
internal fun Process.killPid(): Boolean {
    ...
    this.destroy()
    return true
//    val allRelatedPids = getAllPids(pid)
//    if (FlowShellDebug.isDebug) log(TAG) { "Related pids: $allRelatedPids" }
//    return if (allRelatedPids != null && destroyPids(allRelatedPids)) {
//        true
//    } else {
//        if (FlowShellDebug.isDebug) log(TAG, WARN) { "Couldn't kill all pids, trying Process.destroy()" }
//        this.destroy()
//        false
//    }
}
...
internal data class PidFamily(
    ...
    fun destroyPids(): Boolean {
        var process: Process? = null
//        return try {
//            process = processFactory!!.start("su")
//            val outputStream = OutputStreamWriter(process.outputStream)
//            makeMiniHarvester(process.errorStream).subscribe()
//            makeMiniHarvester(process.inputStream).subscribe()
//            for (p in pids) outputStream.write("kill " + p + LineReader.getLineSeparator())
//            outputStream.write("exit" + LineReader.getLineSeparator())
//            outputStream.flush()
//            outputStream.close()
//            val exitcode = process.waitFor()
//            if (FlowShellDebug.isDebug) Timber.tag(TAG).d("destroyPids(pids=%s) exitcode: %d", pids, exitcode)
//            exitcode == RxProcess.ExitCode.OK
...
//
//fun makeMiniHarvester(inputStream: InputStream?): Single<List<String>> {
//    return Observable
...
//private fun Process.getPids(): PidFamily? {
