...
class CorpseElementsAdapter @Inject constructor() :
    ...
    init {
        ...
        modules.add(TypedVHCreatorMod({ data[it] is CorpseElementHeaderVH.Item }) { CorpseElementHeaderVH(it) })
        modules.add(TypedVHCreatorMod({ data[it] is CorpseElementFileVH.Item }) { CorpseElementFileVH(it) })
    }
