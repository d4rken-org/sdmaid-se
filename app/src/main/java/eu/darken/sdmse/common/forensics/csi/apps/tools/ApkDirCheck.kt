...
class ApkDirCheck @Inject constructor(
    ...
    private suspend fun checkApkName(areaInfo: AreaInfo, name: String): CSISubProcessor.Result {
        val owners = mutableSetOf<Owner>()
        var hasKnownUnknownOwner = false

        val apk = LocalPath.build(areaInfo.file as LocalPath, name)
        val baseInfo = pkgOps.viewArchive(apk, 0) ?: return CSISubProcessor.Result()
    ...
    abstract class DIM {
        @Binds @IntoSet abstract fun mod(mod: ApkDirCheck): CSISubProcessor
    }
