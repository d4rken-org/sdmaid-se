...
@Reusable
class SystemPrivAppModule @Inject constructor(
    ...
    override suspend fun secondPass(firstPass: Collection<DataArea>): Collection<DataArea> {
        ...
        return firstPass
            .filter { it.type == DataArea.Type.SYSTEM && it.hasFlags(DataArea.Flag.PRIMARY) }
            .mapNotNull { area ->
                val path = LocalPath.build(area.path as LocalPath, "priv-app")
    ...
    abstract class DIM {
        @Binds @IntoSet abstract fun mod(mod: SystemPrivAppModule): DataAreaModule
    }

    companion object {
        val TAG: String = logTag("DataArea", "Module", "SystemPrivApp")
    }
}