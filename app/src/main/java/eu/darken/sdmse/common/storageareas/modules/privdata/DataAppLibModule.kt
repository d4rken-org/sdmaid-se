...
@Reusable
class DataAppLibModule @Inject constructor(
    ...
    override suspend fun secondPass(firstPass: Collection<StorageArea>): Collection<StorageArea> {
        ...
        return firstPass
            ...
            .mapNotNull { area ->
                val path = LocalPath.build(area.path as LocalPath, "app-lib")
    ...
    abstract class DIM {
        @Binds @IntoSet abstract fun mod(mod: DataAppLibModule): DataAreaModule
    }

    companion object {
        val TAG: String = logTag("StorageArea", "Module", "DataAppLib")
    }
}