...
class PrivateDataModule @Inject constructor(
     ...
     */
    private suspend fun getMirrored(gateway: LocalGateway, allAreas: Collection<StorageArea>): Collection<StorageArea> {
        val dataAreas = allAreas.filter { it.type == StorageArea.Type.DATA }
    ...
    private suspend fun determineLegacy(
        ...
        allAreas: Collection<StorageArea>
    ): Collection<StorageArea> {
        val dataStorages = allAreas.filter { it.type == StorageArea.Type.DATA }
        ...
        val resultAreas = mutableSetOf<StorageArea>()
        ...
        dataStorages
            .map { baseArea ->
                ...
                when {
                    ...
                    baseArea.hasFlags(StorageArea.Flag.PRIMARY) && !userManager2.hasMultiUserSupport -> {
                        StorageArea(
                            type = StorageArea.Type.PRIVATE_DATA,
    ...
    companion object {
        val TAG: String = logTag("StorageArea", "Module", "PrivateData")
