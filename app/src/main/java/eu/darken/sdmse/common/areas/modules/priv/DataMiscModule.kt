...
@Reusable
class DataMiscModule @Inject constructor(
    ...
    override suspend fun secondPass(firstPass: Collection<DataArea>): Collection<DataArea> {
        ...
        return firstPass
            ...
            .map { parentArea ->
                DataArea(
                    type = DataArea.Type.DATA_MISC,
                    path = parentArea.path.child("misc"),
    ...
    abstract class DIM {
        @Binds @IntoSet abstract fun mod(mod: DataMiscModule): DataAreaModule
    }

    companion object {
        val TAG: String = logTag("DataArea", "Module", "Data", "Misc")
    }
}