...
//
//internal class SystemAppModule(storageFactory: StorageFactory) : StorageFactoryModule(storageFactory) {
//    public override fun build(storageMap: Map<Location, Collection<Storage>>): Collection<Storage> {
//        val file: SDMFile = JavaFile.build(Environment.getRootDirectory(), "app")
//        val mount = FileOpsHelper.findMount(mounts, file) ?: return emptySet()
//
//        return Storage(
//                location = Location.SYSTEM_APP,
//                mount = mount,
//                file = file
//        ).let { setOf(it) }
//    }
