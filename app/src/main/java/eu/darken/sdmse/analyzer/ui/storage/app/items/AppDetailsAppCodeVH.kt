...
import eu.darken.sdmse.databinding.AnalyzerAppDetailsVhAppcodeBinding


class AppDetailsAppCodeVH(parent: ViewGroup) :
    AppDetailsAdapter.BaseVH<AppDetailsAppCodeVH.Item, AnalyzerAppDetailsVhAppcodeBinding>(
        R.layout.analyzer_app_details_vh_appcode,
        parent
    ) {

    override val viewBinding = lazy { AnalyzerAppDetailsVhAppcodeBinding.bind(itemView) }

    override val onBindData: AnalyzerAppDetailsVhAppcodeBinding.(
        item: Item,
        payloads: List<Any>
    ) -> Unit = binding { item ->
