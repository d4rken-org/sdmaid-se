...
/**
 * Lead-in above the footer strip, separating the card's last content line from the timestamp and
 * Discard. Sits outside [DASHBOARD_CUTOUT_DEPTH] so the footer's own content stays exactly on the
 * notch it flanks.
 */
internal val DASHBOARD_HERO_FOOTER_TOP_GAP = 8.dp

/**
 * What the dock reserves for a hero it has not measured yet, for the frame between the card first
 * composing and its size being reported back. Purely a starting value for that animation — it
 * constrains no layout, and the card is free to settle anywhere above or below it.
 */
internal val DASHBOARD_HERO_INITIAL_CARD_HEIGHT = 140.dp

internal val DASHBOARD_HERO_HORIZONTAL_MARGIN = 12.dp

/** Reserved dock height for a hero of [cardHeight]: bar + gap + card (the FAB cradles between). */
internal fun dashboardDockHeightWithHero(cardHeight: Dp): Dp =
    DASHBOARD_BAR_HEIGHT + DASHBOARD_HERO_BAR_GAP + cardHeight
