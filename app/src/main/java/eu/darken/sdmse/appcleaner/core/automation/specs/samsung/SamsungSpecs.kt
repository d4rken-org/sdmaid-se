...
class SamsungSpecs @Inject constructor(
    ...
    override suspend fun isResponsible(pkg: Installed): Boolean {
        if (settings.romTypeDetection.value() == SpecRomType.SAMSUNG) return true
        if (deviceDetective.isCustomROM()) return false
        return deviceDetective.isSamsungDevice()
    }
    ...
    abstract class DIM {
        @Binds @IntoSet abstract fun mod(mod: SamsungSpecs): SpecGenerator
    }
