...
class DataVendorCSITest : BaseCSITest() {
    ...
    private val storageDataMisc1 = mockk<DataArea>().apply {
        every { flags } returns emptySet()
        every { type } returns DataArea.Type.DATA_MISC
        every { path } returns LocalPath.build(baseDataPath1, "misc")
    }

    private val storageDataMisc2 = mockk<DataArea>().apply {
        every { flags } returns emptySet()
        every { type } returns DataArea.Type.DATA_MISC
        every { path } returns LocalPath.build(baseDataPath2, "misc")
    }

    private val storageDataVendor1 = mockk<DataArea>().apply {
        every { flags } returns emptySet()
        every { type } returns DataArea.Type.DATA_VENDOR
        every { path } returns LocalPath.build(baseDataPath1, "vendor")
    }

    private val storageDataVendor2 = mockk<DataArea>().apply {
        every { flags } returns emptySet()
        every { type } returns DataArea.Type.DATA_VENDOR
        every { path } returns LocalPath.build(baseDataPath2, "vendor")
    }
    ...
    private val bases = setOf(
        storageDataVendor1.path,
        storageDataVendor2.path,
    )
    ...
    private fun getProcessor() = DataVendorCSI(
        areaManager = areaManager,
    )

    @Test override fun `test jurisdiction`() = runTest {
        getProcessor().assertJurisdiction(DataArea.Type.DATA_VENDOR)
    }
    ...
    @Test override fun `determine area successfully`() = runTest {
        ...
        for (base in bases) {
            ...
            processor.identifyArea(testFile1)!!.apply {
                type shouldBe DataArea.Type.DATA_VENDOR
    ...
    @Test override fun `fail to determine area`() = runTest {
        ...
        for (base in setOf(storageData1, storageData2).map { it.path as LocalPath }) {
            ...
            processor.identifyArea(base.child("system_ce", rngString)) shouldBe null
            processor.identifyArea(base.child("system", rngString)) shouldBe null
            processor.identifyArea(base.child("vendor", rngString)) shouldNotBe null
        }
    }
