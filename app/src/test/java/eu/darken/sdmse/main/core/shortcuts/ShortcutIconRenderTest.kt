...
class ShortcutIconRenderTest : BaseTest() {
    ...
    @Test
    fun `every clean shortcut tool has one of the shortcut drawables and it renders`() {
        val known = shortcutIcons().values.toSet()
        OneTapCleaner.ONECLICK_TYPES.forEach { type ->
            assertTrue("$type has no ic_shortcut_* icon", known.contains(type.cleanShortcutIconRes))
            assertTrue("$type icon rendered blank", coveredPixels(type.cleanShortcutIconRes) > 0)
        }
    }
}
