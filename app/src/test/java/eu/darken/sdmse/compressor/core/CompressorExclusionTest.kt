...
class CompressorExclusionTest : BaseTest() {
    ...
    @Test
    fun `PathExclusion with COMPRESSOR tag is created correctly`() {
        ...
        exclusion.tags shouldContain Exclusion.Tag.COMPRESSOR
    ...
    fun `PathExclusion id is unique per path`() {
        ...
        val exclusion1 = PathExclusion(path = path1, tags = setOf(Exclusion.Tag.COMPRESSOR))
        val exclusion2 = PathExclusion(path = path2, tags = setOf(Exclusion.Tag.COMPRESSOR))
    ...
    fun `PathExclusion matches exact path`() = runTest {
        ...
        val exclusion = PathExclusion(path = targetPath, tags = setOf(Exclusion.Tag.COMPRESSOR))
    ...
    fun `PathExclusion matches child paths when excluding directory`() = runTest {
        ...
        val exclusion = PathExclusion(path = dirPath, tags = setOf(Exclusion.Tag.COMPRESSOR))
    ...
    fun `PathExclusion does not match parent paths`() = runTest {
        ...
        val exclusion = PathExclusion(path = childPath, tags = setOf(Exclusion.Tag.COMPRESSOR))
    ...
    @Test
    fun `COMPRESSOR tag exists in Exclusion Tag enum`() {
        val compressorTag = Exclusion.Tag.COMPRESSOR

        compressorTag.name shouldBe "COMPRESSOR"
    }
    ...
    fun `exclusion from image path creates correct PathExclusion`() {
        ...
        // Simulating what Compressor.exclude() does
        val exclusion = PathExclusion(
            path = image.path,
            tags = setOf(Exclusion.Tag.COMPRESSOR),
        )
    ...
    fun `exclusion label is a CaString`() {
        val path = LocalPath.build("/storage/emulated/0/DCIM/Camera/IMG_001.jpg")
        val exclusion = PathExclusion(path = path, tags = setOf(Exclusion.Tag.COMPRESSOR))
    ...
    @Test
    fun `PathExclusion with COMPRESSOR tag differs from GENERAL tag`() {
        val path = LocalPath.build("/storage/emulated/0/DCIM/test.jpg")

        val compressorExclusion = PathExclusion(path = path, tags = setOf(Exclusion.Tag.COMPRESSOR))
        val generalExclusion = PathExclusion(path = path, tags = setOf(Exclusion.Tag.GENERAL))

        compressorExclusion.tags shouldBe setOf(Exclusion.Tag.COMPRESSOR)
